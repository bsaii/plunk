...
| 6 | `plunk-worker` | Cloud Run v2 service + Cloud Tasks queues | Internal-only, OIDC-gated worker service with `min_instance_count=0` and request-based billing. It replaces the Worker Pool's ≈$62–68/month always-on baseline after the documented Cloud Tasks cutover. Nine Cloud Tasks queues retain retries (including meter's 10 attempts) and split email into transactional, workflow, and campaign rate tiers. The legacy pool remains only while `remove_legacy_worker_pool=false`. |
