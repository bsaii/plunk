...
| # | Resource | Type | Notes |
| --- | --- | --- | --- |
| 1 | `plunk-api` | Cloud Run v2 service | `min_instance_count=0` (`var.api_min_instance_count`), `cpu_idle=false` (CPU always allocated while an instance is warm — required by the persistent `rediss://` connection in `apps/api/src/database/redis.ts`, see `api.tf`'s header comment), `1 vCPU`/`512Mi` by default. Still scales to zero when fully idle, but pays for CPU continuously (not just per-request) while an instance is warm — a bit more than pure request-based billing, far less than the old always-on instance. Cold starts cost a few seconds of Node/Prisma boot latency after an idle period; flip `api_min_instance_count` back to 1 in `terraform.tfvars` once real traffic makes an always-warm instance worth it |
