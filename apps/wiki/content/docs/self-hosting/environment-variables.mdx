...
| `S3_REGION`            | No       | AWS region the bucket lives in. Only matters for real S3 (Minio ignores it) — must match the bucket's actual region or uploads fail with a signature/region mismatch error. | `us-east-1` |

### Using real AWS S3 instead of Minio

`terraform/aws/` (repo root) provisions the recommended setup: a **private** S3 bucket fronted by
CloudFront as the public endpoint (CloudFront gets read access via Origin Access Control, scoped
to that one distribution — the bucket itself is never public). Point the app at the bucket and
the Terraform-managed CloudFront domain:

```bash
S3_ENDPOINT=https://s3.<your-bucket-region>.amazonaws.com
S3_ACCESS_KEY_ID=<IAM access key>
S3_ACCESS_KEY_SECRET=<IAM secret key>
S3_BUCKET=<your-bucket-name>
S3_PUBLIC_URL=https://<cloudfront_domain_name output from terraform/aws/>
S3_FORCE_PATH_STYLE=false
S3_REGION=<your-bucket-region>
```

`S3_FORCE_PATH_STYLE=false` and a matching `S3_REGION` both matter for real S3 — Minio doesn't
care about either, but AWS's request signing does. `S3_FORCE_PATH_STYLE` also gates whether the
app manages the bucket's public-read policy itself (see below) — leave it `false` for real AWS S3.

Two things that are easy to miss:

- **IAM permissions**: on startup the app calls `HeadBucket`/`CreateBucket` to make sure the
  bucket exists, plus `PutObject`/`GetObject` per upload. It only calls `PutBucketPolicy` when
  `S3_FORCE_PATH_STYLE=true` (Minio dev) — for real AWS S3 that call is skipped entirely, since
  `terraform/aws/` owns the (CloudFront-scoped) bucket policy instead. If you pre-create the
  bucket yourself (which `terraform/aws/` already does), scope the IAM policy down to
  `s3:GetObject`/`s3:PutObject` on `<bucket-arn>/*` plus `s3:ListBucket` on the bucket itself
  (`<bucket-arn>`, no `/*`) — `HeadBucket` is authorized by `s3:ListBucket`, there is no
  `s3:HeadBucket` action — and skip granting `s3:CreateBucket`/`s3:PutBucketPolicy` entirely.
- **S3 Block Public Access**: irrelevant with the `terraform/aws/` setup above, since the bucket
  is never made public directly — `aws_s3_bucket_public_access_block` blocks all four settings and
  CloudFront's Origin Access Control is the only path in. This only matters if you set the bucket
  policy up by hand instead of using the provided Terraform.
...
Selects how the 5 scheduled maintenance jobs (domain verification, segment counts, and the API request / idempotency-key / email-body cleanup sweeps) are run.

| Variable        | Required | Description                                                                                                                                                                                                                                | Default |
| --------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `QUEUE_BACKEND` | No       | Self-hosted deployments should leave this unset — `bullmq` runs the maintenance jobs as in-process BullMQ repeatable jobs against the worker container from `docker-compose.yml`, exactly as today. `cloud-tasks` is reserved for Plunk's own GCP deployment (Cloud Scheduler + a dedicated Cloud Run Job) and is not a supported self-hosting configuration. | `bullmq` |
| `REALTIME_QUEUE_BACKEND` | No | Leave unset when self-hosting. `cloud-tasks` is reserved for Plunk's internal GCP worker-service rollout and is activated only after its queues and authenticated worker service are deployed. | `bullmq` |
| `CLOUD_TASKS_PROJECT_ID` | No | GCP project that owns the internal Cloud Tasks queues. Not used when `REALTIME_QUEUE_BACKEND` is `bullmq`. | — |
| `CLOUD_TASKS_LOCATION` | No | Region of the internal Cloud Tasks queues. Not used when `REALTIME_QUEUE_BACKEND` is `bullmq`. | — |
| `CLOUD_TASKS_WORKER_URL` | No | Internal worker-service base URL for Cloud Tasks HTTP delivery. Not used when `REALTIME_QUEUE_BACKEND` is `bullmq`. | — |
| `CLOUD_TASKS_AUDIENCE` | No | Expected OIDC token audience at the internal worker service. Not used when `REALTIME_QUEUE_BACKEND` is `bullmq`. | — |
| `CLOUD_TASKS_SERVICE_ACCOUNT_EMAIL` | No | Service account Cloud Tasks uses to authenticate task requests. Not used when `REALTIME_QUEUE_BACKEND` is `bullmq`. | — |
