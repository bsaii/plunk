...
Point the same variables at a real bucket instead of the bundled Minio service:
...
S3_PUBLIC_URL=https://<your-bucket-name>.s3.<your-bucket-region>.amazonaws.com
...
care about either, but AWS's request signing does.

Two things that are easy to miss:

- **IAM permissions**: on startup the app calls `HeadBucket`/`CreateBucket`/`PutBucketPolicy` to
  make sure the bucket exists and is publicly readable, plus `PutObject`/`GetObject` per upload.
  If you pre-create the bucket yourself, you can scope the IAM policy down to just
  `s3:GetObject`/`s3:PutObject`/`s3:HeadBucket` on that bucket and skip granting
  `s3:CreateBucket`/`s3:PutBucketPolicy`.
- **S3 Block Public Access**: new AWS buckets have this on by default, which silently rejects the
  startup `PutBucketPolicy` call (the app logs a warning and keeps going rather than crashing).
  The practical effect: uploaded files 403 for anyone opening `S3_PUBLIC_URL` in a browser or
  email client. Either turn off Block Public Access for this bucket, or — better for a
  production setup — put a CloudFront distribution in front of the bucket instead of making it
  public directly, and set `S3_PUBLIC_URL` to the CloudFront domain.
