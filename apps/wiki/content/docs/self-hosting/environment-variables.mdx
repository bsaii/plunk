...
| `WORKER_HEALTH_PORT` | No | Port the worker process serves its health endpoint (`GET /health`) on, reporting whether the queue consumers have started. The worker is a queue consumer rather than an HTTP server, so this exists purely for container health probes. The all-in-one image pins it to `8081` because the API already owns `8080` in the shared container. Falls back to `PORT`, then `8081`. | `8081`  |
